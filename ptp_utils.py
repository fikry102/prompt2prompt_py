...
def text2image_ldm_stable(
    ...
    latent, latents = init_latent(latent, model, height, width, generator, batch_size)#set initial latent noise
...
def register_attention_control(model, controller):
    def ca_forward(self, place_in_unet):
        ...
        def forward(x, context=None, mask=None):
            ...
            h = self.heads#num of heads
            q = self.to_q(x)#deep spatial features of the noisy image -> query matrix
            is_cross = context is not None
            context = context if is_cross else x
            k = self.to_k(context)#the textual embedding -> key matrix and value matrix
            v = self.to_v(context)
            ...
            attn = controller(attn, is_cross, place_in_unet)# __call__ method: "Controlling the Cross-attention",
